use rebrickerable;
ALTER VIEW  rebrickerable. analytics_main as

select s.set_num, s.name as set_name, s.year, s.theme_id,s.num_parts as  num_parts, t.name as theme_name, t.parent_id, p.name as parent_theme_name,
case 
	when s.year between 1901 and 2000 then '20th_Century'
	when s.year between 2001 and 2100 then '21st_Century'
end
as Century
from rebrickerable .sets s
left join rebrickerable.themes t
	on s.theme_id = t.id
left join rebrickerable.themes p
	on t.parent_id = p.id;
...
select * from rebrickerable .analytics_main;


select theme_name, sum(num_parts) as total_num_parts 
from rebrickerable.analytics_main
...
from 
	(
		select
			inv.color_id, inv.inventory_id, inv.part_num, inv.quantity as quantity, inv.is_spare, c.name as color_name, c.rgb, p.name as part_name, p.part_material, pc.name as category_name
